...
let handler = async (m, { conn, command, args }) => {
  ...
  if (global.DATABASE._data.users[m.sender].exp >= xpperlimit * count) {
    ...
    conn.reply(m.chat, `-${xpperlimit * count} Coin\n+ ${count} Limit`, m)
  } else conn.reply(m.chat, `Coin tidak mencukupi untuk membeli ${count} limit`, m)
}
handler.help = ['buy <jumlah limit>', 'buy', 'buyall']
handler.tags = ['xp']

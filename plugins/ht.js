let handler = async (m, { conn, text }) => {
  ...
  if (m.chat in conn.hartatahta) throw 'Masih ada yang sedang membuat teks Harta Tahta di chat ini, sila tunggu sampai selesai'
  else conn.hartatahta[m.chat] = true
  m.reply('Sedang membuat teks Harta Tahta')
  try {
    let img = await ht(text ? text : ':v')
    conn.sendFile(m.chat, img, 'Selesai ✓', m)
  } finally {

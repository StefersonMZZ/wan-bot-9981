let handler = async (m, { conn, text }) => {
  ...
  if (m.chat in conn.hartatahta) throw 'Masih ada yang sedang membuat teks di chat ini, sila tunggu sebentar'
  else conn.hartatahta[m.chat] = true
  m.reply('Sedang membuat teks\nTolong tunggu selama 1 minit*')
  try {
    let img = await ht(text ? text : ':v')
    conn.sendFile(m.chat, img, 'Harta Tahta.png', 'Ini teksnya', m)
  } finally {

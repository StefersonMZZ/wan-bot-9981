...
if (!opts['test']) setInterval(() => {
  conn.logger.info('Sedang save database')
  if (JSON.stringify(global.DATABASE.data) == lastJSON) conn.logger.info('Database terpulang')
  else {
    global.DATABASE.save()
    conn.logger.info('Database sudah di save')
  ...
  try {
    ...
    let _user = global.DATABASE._data.users[m.sender]
      ...
      let _prefix = plugin.customPrefix ? plugin.customPrefix : conn.prefix ? conn.prefix : global.prefix
        ...
        if (xp > 99) m.reply('Dilarang cheat') // Hehehe
        else m.exp += xp
        if (!isPrems && global.DATABASE._data.users[m.sender].limit < m.limit * 1 && plugin.limit) {
          this.reply(m.chat, `Limit anda habis, sila beli melalui *${usedPrefix}buy*`, m)
...
Dikesan @${m.participant.split`@`[0]} telah memadam mesej 😂

Untuk mematikan fitur ini, sila taip *.enable delete*
`.trim(), m.message, {
    contextInfo: {
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: 'Fitur ini hanya dapat digunakan oleh owner robot',
    owner: 'Fitur ini hanya dapat digunakan oleh owner robor',
    mods: 'Fitur ini hanya dapat digunakan oleh moderator robot',
    premium: 'Fitur ini hanya untuk member premium',
    group: 'Fitur ini hanya dapat digunakan di grup',
    private: 'Fitur ini hanya dapat digunakan di chat pribadi',
    admin: 'Fitur ini hanya untuk admin grup',
    botAdmin: 'Jadikan robot sebagai admin untuk menggunakan fitur ini',
    unreg: 'Sila daftar untuk menggunakan fitur ini dengan cara menaip:\n\n*#daftar nama.umur*\n\nContoh: *#daftar Wan.13*'
  }[type]

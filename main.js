  ...
  try {
    ...
    m.exp += 1
      ...
      let _prefix = plugin.customPrefix ? plugin.customPrefix : conn.prefix ? conn.prefix : global.prefix
        ...
        if (xp > 99) m.reply('Cheat -_-') // Hehehe
        else m.exp += xp
...
conn.welcome = 'Hai, @user! Selamat datang di grup @subject\n\nAnda wajib baca info grup!'
conn.bye = 'Selamat tinggal @user!\n\nSesiapa tolong add satu orang untuk pengganti'
...
Untuk mematikan fitur ini, taip .enable delete
`.trim(), m.message, {
    contextInfo: {
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    owner: 'Fitur ini hanya dapat digunakan oleh owner robot',
    ...
    private: 'Fitur ini hanya dapat digunakan di chat personal',

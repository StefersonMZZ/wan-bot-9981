    ...
    m.exp += 1
      ...
      let _prefix = plugin.customPrefix ? plugin.customPrefix : conn.prefix ? conn.prefix : global.prefix
        ...
        if (!isPrems && global.DATABASE._data.users[m.sender].limit < m.limit * 1 && plugin.limit) {
          this.reply(m.chat, `Limit anda habis, sila beli dengan taip ${usedPrefix}buy`, m)
...
conn.welcome = 'Hai @user!\nSelamat datang di grup @subject. Anda wajib baca deksrisi grup'
...
Untuk mematikan fitur ini, sila taip
.enable delete
`.trim(), m.message, {
    contextInfo: {
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    mods: 'Fitur ini hanya dapat digunakan oleh Moderator',
